...
test('deployFirmwareBootstrap rejects RP2350 UF2 bootloader targets with Pico 2 guidance', async () => {
  await withTempDir((dir) => {
    ...
    assert.equal(result.status, 'uf2_bootloader');
    assert.match(result.message, /Pico 2/i);
    assert.match(result.message, /raspberry_pi_pico2/i);
    assert.equal(existsSync(resolve(targetDir, 'boot.py')), false);
  });
});

test('deployFirmwareBootstrap rejects RP2040 UF2 bootloader targets with Pico guidance', async () => {
  await withTempDir((dir) => {
    const artifact = buildArtifactInTempDir(dir);
    const targetDir = resolve(dir, 'target-rp2040');
    mkdirSync(targetDir, { recursive: true });
    writeFileSync(
      resolve(targetDir, 'INFO_UF2.TXT'),
    ...
    assert.equal(result.ok, false);
    assert.equal(result.status, 'uf2_bootloader');
    assert.match(result.message, /Pico \/ RP2040/i);
    assert.match(result.message, /raspberry_pi_pico\//i);
    assert.equal(existsSync(resolve(targetDir, 'code.py')), false);
  });
});
...
test('deployFirmwareBootstrap copies managed files to a CircuitPython target without deleting unrelated files', async () => {
  await withTempDir((dir) => {
    ...
    assert.match(result.message, /USB storage is expected to disappear/i);
  });
});
