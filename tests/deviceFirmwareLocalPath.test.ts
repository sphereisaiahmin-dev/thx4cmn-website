...
const withLocalFirmwareWorkspace = async (
  ...
  const packagePath = join(distDir, 'hx01-firmware-0.9.6-direct.json');
  ...
  process.env[DEVICE_FIRMWARE_LOCAL_PACKAGE_PATH_ENV] = 'dist/hx01-firmware-0.9.6-direct.json';
...
test('local firmware package metadata exposes the configured package', async () => {
  await withLocalFirmwareWorkspace(async ({ packageSha256 }) => {
    ...
    assert.equal(metadata.releaseRank, 50906);
...
test('latest firmware lookup can build an unavailable fallback response', async () => {
  ...
  assert.equal(response.currentVersion, '0.9.5');
...
test('latest firmware lookup accepts canonical hx01 even when manifest metadata differs', async () => {
  const response = await buildLatestFirmwareResponse({
    ...
    manifest: {
      ...
      latestVersion: '0.9.6',
      latestReleaseRank: 50906,
      releases: [
        {
          version: '0.9.6',
          releaseRank: 50906,
          downloadUrl: 'https://example.r2.cloudflarestorage.com/updates/hx01-firmware-0.9.6-direct.json',
          sha256: 'c'.repeat(64),
  ...
  assert.equal(response.targetVersion, '0.9.6');
  assert.equal(response.latestVersion, '0.9.6');
});
