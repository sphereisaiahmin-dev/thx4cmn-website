...
export async function GET(request: Request) {
  ...
  if (!isR2MusicKey && !(process.env.NODE_ENV !== 'production' && isLocalFixtureKey)) {
    return NextResponse.json({ error: 'Invalid key.' }, { status: 400, headers: NO_STORE_HEADERS });
  }

  if (process.env.NODE_ENV !== 'production' && isLocalFixtureKey) {
    return NextResponse.json({ url: toFixtureSignedUrl(key) }, { headers: NO_STORE_HEADERS });
  }
